...
void StructDeclaration::ParseAttributes(Symbol* sym)
{
  ...
  for (;;) {
    if (lastst == kw_attribute)
      ...
      NextToken();
      sym->tp->alignment = (int)GetIntegerExpression(&pnd, sym, 0).low;
    }
    else
      break;
  }
...
int StructDeclaration::Parse(TABLE* table, int ztype, Symbol** sym)
{
  ...
  // Else there was no tag identifier
    ...
    sprintf_s(nmbuf, sizeof(nmbuf), "__noname_tag%d", cnt);
    cnt++;
    ...
    if (lastst==kw_align) {
      NextToken();
      tp->alignment = (int)GetIntegerExpression(&pnd,sp,0).low;
    }
...
for (;;) {
  if (lastst == kw_attribute)
    ...
    NextToken();
    sp->tp->alignment = (int)GetIntegerExpression(&pnd,sp,0).low;
  }
  else
    break;
}
