...
BasicBlock *BasicBlock::Blockize(OCODE *start)
{
	...
	for (ip = start; ip; ip = ip2) {
		...
		ip2 = ip->fwd;
		if (ip->opcode != op_label && ip->opcode != op_remark && ip->opcode != op_rem2 && 
				ip->opcode != op_hint && ip->opcode != op_hint2)
			pb->length++;
	...
	pb->next = nullptr;
	dfs.printf((char *)"%s: ", (char *)currentFn->sym->name->c_str());
	dfs.printf((char *)"%d basic blocks\n", num);
...
//				}
	...
	gen->resetPtr();
	kill->resetPtr();
	dfs.printf((char *)"%d: ", num);
	for (nn = 0; nn < gen->NumMember(); nn++)
		dfs.printf((char *)"g%d ", gen->nextMember());
	dfs.printf(" || ");
	for (nn = 0; nn < kill->NumMember(); nn++)
		dfs.printf((char *)"k%d ", kill->nextMember());
	dfs.printf("\n");
	dfs.printf("Edges to: ");
	LiveOut->copy(*LiveIn);
	if (ohead!=nullptr)
