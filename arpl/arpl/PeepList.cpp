...
// Used to detect when safe to remove ENTER and LEAVE
		...
		if (ip->opcode != op_label && ip->opcode != op_rem) {
			if (ip->oper1) {
				if (ip->oper1->preg > 0 && !(IsArgReg(ip->oper1->preg) || IsTempReg(ip->oper1->preg)))
					return (false);
				if (ip->oper1->sreg > 0 && !(IsArgReg(ip->oper1->sreg) || IsTempReg(ip->oper1->sreg)))
					return (false);
			}
			if (ip->oper2) {
				if (ip->oper2->preg > 0 && !(IsArgReg(ip->oper2->preg) || IsTempReg(ip->oper2->preg)))
					return (false);
				if (ip->oper2->sreg > 0 && !(IsArgReg(ip->oper2->sreg) || IsTempReg(ip->oper1->sreg)))
					return (false);
			}
			if (ip->oper3) {
				if (ip->oper3->preg > 0 && !(IsArgReg(ip->oper3->preg) || IsTempReg(ip->oper3->preg)))
					return (false);
				if (ip->oper3->sreg > 0 && !(IsArgReg(ip->oper3->sreg) || IsTempReg(ip->oper3->sreg)))
					return (false);
			}
			if (ip->oper4) {
				if (ip->oper4->preg > 0 && !(IsArgReg(ip->oper4->preg) || IsTempReg(ip->oper4->preg)))
					return (false);
				if (ip->oper4->sreg > 0 && !(IsArgReg(ip->oper4->sreg) || IsTempReg(ip->oper4->sreg)))
					return (false);
			}
		}
	}
...
void PeepList::EliminateUnreferencedLabels()
{
	...
	for (p = head; p; p = p->fwd) {
		if (p->opcode == op_label)
			p->remove = false;
		if (p->opcode == op_label && !p->isReferenced && !p->str) {
			p->MarkRemove();
...
void PeepList::OptInstructions()
{
	...
	for (ip = head; ip != NULL; ip = ip->fwd)
	{
		if (!ip->remove) {
			...
			case op_br:
			case op_beq:	ip->OptBeq(); ip->OptIncrBranch(); break;
			case op_bne:	ip->OptBne(); ip->OptIncrBranch(); break;
			case op_ble:	ip->OptIncrBranch(); break;
			case op_blt:	ip->OptBlt();  ip->OptIncrBranch(); break;
			case op_bnez:	ip->OptIncrBranch(); break;
			case op_bltz:	ip->OptIncrBranch(); break;
			case op_blez:	ip->OptIncrBranch(); break;
			case op_bgt:	ip->OptBgt(); break;
			case op_bge:	ip->OptBge(); break;
...
void PeepList::Dump(char *msg)
{
	...
	for (ip = head; ip; ip = ip->fwd) {
		...
		if (ip->opcode == op_label) {
			if (ip->str)
				dfs.printf((char *)"%s%s:", (char*)ip->oper2, (char*)ip->oper1);
			else
				dfs.printf((char *)"%s%d:", (char*)ip->oper2, (int)ip->oper1);
		}
		else if (insn)
			dfs.printf((char *)"   %s ", (char *)insn->mnem);
		else
