...
int IdentifyKeyword()
{
	...
	if (p[0]=='c') {	// case,catch,char,class,const,continue,coroutine
		...
		if (p[1] == 'o' && p[2] == 'r' && p[3] == 'o' && p[4] == 'u' && p[5] == 't' && p[6] == 'i' && p[7] == 'n' && p[8]== 'e' && p[9] == '\0')
			return lastst = kw_coroutine;
	}
	...
	if (p[0] == 'y') {
		if (p[1] == 'i' && p[2] == 'e' && p[3] == 'l' && p[4] == 'd' && p[5] == '\0') {
			return lastst = kw_yield;
		}
	}


	// __cdecl,__asm,__int64,__int32,__int16,__int8,__check,__exception,__task,__unordered,__leafs,__attribute__
	// __no_temps __no_parms __floatmax __mulf __bmap __save_context __sync
	...
	if (p[0]=='_' && p[1]=='_') {
		...
		if (strcmp(&p[2], "sync") == 0)
			return (lastst = kw_sync);
	}
